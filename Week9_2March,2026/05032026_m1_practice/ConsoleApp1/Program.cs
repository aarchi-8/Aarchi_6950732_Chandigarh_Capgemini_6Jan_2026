...
public class LibrarySystem : ILibrarySystem
{
    ...
    {
        return BookStorage.GroupBy(b => b.Key.Category)
            .Select(g => (g.Key, g.Sum(b => b.Key.Price * b.Value))).ToList();
    }

    public List<(string, int, int)> BooksInfo()
    {
        return BookStorage.Select(b => (b.Key.Title, b.Value, b.Key.Price)).ToList();
    }

    public List<(string, string, int)> CategoryAndAuthorWithCount()
    {
        return BookStorage.GroupBy(b => (b.Key.Category, b.Key.Author))
            .Select(g => (g.Key.Category, g.Key.Author, g.Sum(b => b.Value))).ToList();
    }
    class Solution
    {
